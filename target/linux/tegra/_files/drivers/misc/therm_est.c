...
static void therm_est_set_limits(struct therm_estimator *est, int low, int high)
{
	long low_temp = low, high_temp = high;
	int trip_temp, passive_low_temp = high;
	enum thermal_trip_type trip_type;
	int hysteresis, zone_temp;
	int i;

	zone_temp = est->thz->temperature;
	for (i = 0; i < est->thz->num_trips; i++) {
...
static void therm_est_work_func(struct work_struct *work)
{
	...
	if (est->thz && ((est->cur_temp < est->low_limit) ||
		...
		//therm_est_set_limits(est, low, high);
	}
...
static int therm_est_set_trips(void *of_data, int low, int high)
{
	struct therm_estimator *est = (struct therm_estimator *)of_data;
