...
static void fan_update_target_pwm(struct fan_dev_data *fan_data, int val)
{
	if (fan_data) {
		...
		if (fan_data->next_target_pwm != fan_data->fan_cur_pwm) {
			cancel_delayed_work_sync(&fan_data->fan_ramp_work);
			queue_delayed_work(fan_data->workqueue,
...
static int pwm_fan_probe(struct platform_device *pdev)
{
	...
	if (!node) {
		dev_err(&pdev->dev, "dev of_node NULL\n");
		return -EINVAL;
	}

	data_node = of_parse_phandle(node, "shared_data", 0);
	if (!data_node) {
		dev_err(&pdev->dev, "PWM shared data node NULL, parse phandle failed\n");
		return -EINVAL;
	}
	...
	if (gpio_is_valid(pwm_fan_gpio)) {
        ...
        //devm_gpio_request()
        if (err) {
            dev_err(&pdev->dev, "GPIO request failed GPIO num: %d\n", pwm_fan_gpio);
            //pr_err("FAN: gpio request failed GPIO num: %d\n", pwm_fan_gpio);
            err = -EINVAL;
            goto gpio_request_fail;
        }
        else {
            dev_info(&pdev->dev, "GPIO request success GPIO num: %d\n", pwm_fan_gpio);
            //pr_info("FAN: gpio request success GPIO num: %d\n", pwm_fan_gpio);
            //fan_data->pwm_gpio = pwm_fan_gpio;
        }
    }
    else {
		if (pwm_fan_gpio != -EPROBE_DEFER) {
			dev_info(&pdev->dev, "Failed to retrieve 'pwm_gpio' from 'shared_data' in dts\n");
			//pr_info("FAN: Failed to retrieve 'pwm_gpio' from 'shared_data' in dts\n");
			of_err |= pwm_fan_gpio;
		}
		else {
			err = -EPROBE_DEFER;
	...
	if (gpio_is_valid(tach_gpio)) {
        //fan_data->tach_gpio = tach_gpio;
		value = tach_gpio;	
	}
    else {
		if (tach_gpio != -EPROBE_DEFER) {
			//fan_data->tach_gpio = -1;
			dev_info(&pdev->dev, "can't find tach_gpio\n");
	...
	if (IS_ERR_OR_NULL(fan_data->fan_reg)) {
		dev_err(&pdev->dev, "FAN: coudln't get the regulator\n");
